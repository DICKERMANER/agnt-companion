...
def get_or_create_user_bundle(db: Session, user_id: str) -> tuple[User, Companion]:
    ...
    if not companion:
        companion = Companion(user_id=user.id, favorability_score=0, relationship_stage="cold")
        db.add(companion)
        db.commit()
...
async def webhook_chat(payload: ChatRequest, db: Session = Depends(get_db)) -> ChatResponse:
    ...
    # 動態 Prompt 注入
    system_prompt = build_dynamic_system_prompt(companion.relationship_stage)
